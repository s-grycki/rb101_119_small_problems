...
# The given solution was performing integer division when it wanted float
# division. This meant that division would occur without the potential for
# remianders. The fdiv method is one way to perform float division, but we
# could have simply converted one of the numbers to a float too

# FURTHER EXPLORATION: There's still an issue that can come up with division
# If none of the documents have the search term then there will be a divide
# by zero as a float. This won't give an error like with integers, but it will
# return 'Infinity' if the numerator isn't zero, or 'NaN' if it is. The 
# solution is to add a guard clause to return 0 in case none of the documents
# have the term
